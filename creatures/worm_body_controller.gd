...
var _path_points_world: PackedVector3Array = PackedVector3Array()

var _previous_head_position_world: Vector3 = Vector3.ZERO
...
func rebuild() -> void:
	...
	_path_points_world.clear()
	_has_head_position = false
	_preview_distance_m = 0.0

	var initial_head_position: Vector3 = creature_mesh.global_position
	var initial_forward: Vector3 = (
		-creature_mesh.global_transform.basis.z
	).normalized()

	_initialize_straight_path(
...
func _update_preview_motion(delta: float) -> void:
	...
	var safe_radius_m: float = maxf(preview_path_radius_m, 0.01)
	var circumference_m: float = TAU * safe_radius_m
	var angle_rad: float = _preview_distance_m / circumference_m * TAU

	var preview_origin: Vector3 = creature_mesh.global_position

	var head_position_world: Vector3 = preview_origin + Vector3(
		cos(angle_rad) * safe_radius_m,
		sin(angle_rad * 2.0) * preview_vertical_wave_m,
		sin(angle_rad) * safe_radius_m
	)

	set_head_position(head_position_world)

func set_head_position(world_position: Vector3) -> void:
	if _worm_data == null:
		return
	...
	if not _has_head_position:
		...
		_initialize_straight_path(world_position, initial_direction)
		return

	var movement: Vector3 = (
		world_position - _previous_head_position_world
	)
	...
	var movement_direction: Vector3 = movement / movement_distance_m
	var remaining_distance_m: float = movement_distance_m
	var sample_position: Vector3 = _previous_head_position_world
	...
	if not new_points.is_empty():
		var combined_points: PackedVector3Array = PackedVector3Array()

		for point: Vector3 in new_points:
			combined_points.append(point)

		for point: Vector3 in _path_points_world:
			combined_points.append(point)

		_path_points_world = combined_points

	_previous_head_position_world = world_position
	_trim_path()

func _initialize_straight_path(
	head_position_world: Vector3,
	forward_direction_world: Vector3
) -> void:
	_path_points_world.clear()

	var safe_forward: Vector3 = forward_direction_world.normalized()
	...
	var point_count: int = maxi(
		...
		ceili(required_path_length_m / path_point_spacing_m) + 1
	)

	for point_index: int in range(point_count):
		_path_points_world.append(
			head_position_world
			- safe_forward * path_point_spacing_m * point_index
		)

	_previous_head_position_world = head_position_world
...
func _trim_path() -> void:
	...
	var max_point_count: int = maxi(
		_worm_data.segment_count + 2,
		ceili(max_path_length_m / path_point_spacing_m) + 1
	)

	while _path_points_world.size() > max_point_count:
		_path_points_world.remove_at(_path_points_world.size() - 1)

func _update_body_mesh() -> void:
	if _worm_data == null or _path_points_world.size() < 2:
		return

	var body_points_world: PackedVector3Array = PackedVector3Array()

	for ring_index: int in range(_worm_data.segment_count + 1):
		var t: float = float(ring_index) / float(_worm_data.segment_count)
		var distance_behind_head_m: float = _worm_data.length_m * t

		body_points_world.append(
			_get_path_position_at_distance(distance_behind_head_m)
		)

	var mesh_inverse_transform: Transform3D = (
		creature_mesh.global_transform.affine_inverse()
	)

	var body_points_local: PackedVector3Array = PackedVector3Array()

	for point_world: Vector3 in body_points_world:
		body_points_local.append(
			mesh_inverse_transform * point_world
		)

	_generator.update_worm_mesh(_worm_data, body_points_local)
...
func _get_path_position_at_distance(
	...
	for point_index: int in range(_path_points_world.size() - 1):
		var from: Vector3 = _path_points_world[point_index]
		var to: Vector3 = _path_points_world[point_index + 1]
		...
		if walked_distance_m + segment_length_m >= target_distance_m:
			...
			var t: float = local_distance_m / maxf(segment_length_m, 0.0001)
			return from.lerp(to, t)

		walked_distance_m += segment_length_m

	return _path_points_world[_path_points_world.size() - 1]
